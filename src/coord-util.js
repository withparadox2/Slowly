var LocationUtil = (function () {
  ...
  LocationUtil.gcj2bd = function (lat, lon) {
    var z = Math.sqrt(lon * lon + lat * lat) + 2.0E-5 * Math.sin(lat * LocationUtil.x_pi);
    var theta = Math.atan2(lat, lon) + 3.0E-6 * Math.cos(lon * LocationUtil.x_pi);
  ...
  LocationUtil.outOfChina = function (wgsLat, wgsLon) {
    return wgsLon >= 72.004 && wgsLon <= 137.8347 ? wgsLat < 0.8293 || wgsLat > 55.8271 : true;
  };
  /*private*/ LocationUtil.delta = function (lat, lon) {
    ...
    dLat = dLat * 180.0 / (a * (1.0 - ee) / (magic * sqrtMagic) * LocationUtil.pi);
    dLon = dLon * 180.0 / (a / sqrtMagic * Math.cos(radLat) * LocationUtil.pi);
    return [dLat, dLon];
  };
  /*private*/ LocationUtil.transformLat = function (lat, lon) {
    var ret = -100.0 + 2.0 * lat + 3.0 * lon + 0.2 * lon * lon + 0.1 * lat * lon + 0.2 * Math.sqrt(Math.abs(lat));
    ret += (20.0 * Math.sin(6.0 * lat * LocationUtil.pi) + 20.0 * Math.sin(2.0 * lat * LocationUtil.pi)) * 2.0 / 3.0;
    ret += (20.0 * Math.sin(lon * LocationUtil.pi) + 40.0 * Math.sin(lon / 3.0 * LocationUtil.pi)) * 2.0 / 3.0;
    ret += (160.0 * Math.sin(lon / 12.0 * LocationUtil.pi) + 320.0 * Math.sin(lon * LocationUtil.pi / 30.0)) * 2.0 / 3.0;
    return ret;
  };
  /*private*/ LocationUtil.transformLon = function (lat, lon) {
    var ret = 300.0 + lat + 2.0 * lon + 0.1 * lat * lat + 0.1 * lat * lon + 0.1 * Math.sqrt(Math.abs(lat));
    ret += (20.0 * Math.sin(6.0 * lat * LocationUtil.pi) + 20.0 * Math.sin(2.0 * lat * LocationUtil.pi)) * 2.0 / 3.0;
    ret += (20.0 * Math.sin(lat * LocationUtil.pi) + 40.0 * Math.sin(lat / 3.0 * LocationUtil.pi)) * 2.0 / 3.0;
    ret += (150.0 * Math.sin(lat / 12.0 * LocationUtil.pi) + 300.0 * Math.sin(lat / 30.0 * LocationUtil.pi)) * 2.0 / 3.0;
    return ret;
  };
  LocationUtil.mercator2LatLon = function (x, y) {
    var lon = x / 2.003750834E7 * 180.0;
    var lat = y / 2.003750834E7 * 180.0;
    lat = 57.29577951308232 * (2.0 * Math.atan(Math.exp(lat * 3.141592653589793 / 180.0)) - 1.5707963267948966);
    var latLng = [lat, lon];
    return latLng;
  };
  LocationUtil.latLon2Mercator = function (lat, lon) {
    var x = lon * 2.003750834E7 / 180.0;
    var y = Math.log(Math.tan((90.0 + lat) * 3.141592653589793 / 360.0)) / 0.017453292519943295;
